...
if (SAVE_ALL){
  ...
  for (idx_1 in 1:(nlevels(dds$condition)-1)) {
    for (idx_2 in (idx_1+1):nlevels(dds$condition)){
      ...
      write.csv(res, file=paste("example//", levels(dds$condition)[idx_1], '.vs.', levels(dds$condition)[idx_2],'.csv', sep =''))
      print(paste('Result saved to ', paste("example//", levels(dds$condition)[idx_1], '.vs.', levels(dds$condition)[idx_2],'.csv', sep ='')))
  }
  }
}else{
